...
for p in products:
    ...
    exclude_keywords = [
        ...
        'ketchups', 'pickles', 'achaar', 'digestive', 'nescafe', 'nestle', 'orco'
    ]
    if any(keyword in p_lower for keyword in exclude_keywords):
        continue
    agro_products.append(p)

final_products = set(['Spices (All)', 'Dry Fruits & Nuts (All)', 'Cement', 'TMT Bars & Steel'])

# Keywords to replace with Spices (All)
spice_keywords = ['masala', 'spice', 'pepper', 'cardamom', 'cinnamon', 'clove', 'coriander', 'cumin', 'turmeric', 'ginger powder', 'garlic powder', 'chili', 'chilli', 'mirch', 'ajwain', 'asafoetida', 'hing', 'fennel', 'fenugreek', 'mace', 'nutmeg', 'oregano', 'basil', 'saffron', 'kesar', 'pudina', 'sounth', 'mustard seeds', 'panch phoron']

# Keywords to replace with Dry Fruits & Nuts (All)
dryfruit_keywords = ['almond', 'anjeer', 'apricot', 'cashew', 'chiraunji', 'dates', 'khajoor', 'hazelnut', 'pecan', 'pista', 'prune', 'raisin', 'walnut']

# Sugar keywords to minimize
sugar_keywords = ['sugar', 'icumsa', 'jaggery']

for p in agro_products:
    p_lower = p.lower()
    
    if any(k in p_lower for k in spice_keywords):
        continue
    if any(k in p_lower for k in dryfruit_keywords):
        continue
    if any(k in p_lower for k in sugar_keywords):
        continue
        
    final_products.add(p)

# Also add minimal sugar
final_products.add('Sugar (All Types)')
final_products.add('Jaggery')
final_products.add('Mustard Oil')
final_products.add('Sunflower Oil')
final_products.add('Palm Oil')
final_products.add('Soybean Oil')
final_products.add('Groundnut Oil')
final_products.add('Olive Oil')

final_products_list = sorted(list(final_products))
...
# We are injecting it into the new structure that HAS the search bar
...
# Be careful about the regex replace
...
print(f"Updated successfully with {len(final_products_list)} curated products. Preserved Makhana, Moringa, etc.")
