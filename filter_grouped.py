...
final_products = set(['Cement', 'TMT Bars & Steel', 'Cow Dung'])

# Keywords
rice_keywords = ['rice']
sugar_keywords = ['sugar', 'icumsa', 'jaggery']
spice_keywords = ['masala', 'spice', 'pepper', 'cardamom', 'cinnamon', 'clove', 'coriander', 'cumin', 'turmeric', 'ginger', 'garlic powder', 'chili', 'chilli', 'mirch', 'ajwain', 'asafoetida', 'hing', 'fennel', 'fenugreek', 'mace', 'nutmeg', 'oregano', 'basil', 'saffron', 'kesar', 'pudina', 'sounth', 'mustard seeds', 'panch phoron', 'bay leaf', 'tej patta', 'amchoor', 'star anise']
dryfruit_keywords = ['almond', 'anjeer', 'apricot', 'cashew', 'chiraunji', 'dates', 'khajoor', 'hazelnut', 'pecan', 'pista', 'prune', 'raisin', 'walnut', 'pistachio']

for p in agro_products:
    p_lower = p.lower()
    
    if any(k in p_lower for k in rice_keywords):
        final_products.add('Rice')
        continue
    if any(k in p_lower for k in sugar_keywords):
        final_products.add('Sugar')
        continue
    if any(k in p_lower for k in spice_keywords):
        final_products.add('Spices')
        continue
    if any(k in p_lower for k in dryfruit_keywords):
        final_products.add('Dry Fruits')
        continue
...
print(f"Updated successfully with {len(final_products_list)} grouped products + search bar.")
