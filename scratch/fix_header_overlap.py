...
# Dedicated Executive 3D Pill Action Bar (Placed right below Top Announcement Bar)
...
for filename in html_files:
    ...
    if os.path.exists(fpath):
        ...
        # 1. Remove top-pill-actions-container inside <nav>
        content = re.sub(r'<!-- TOP EXECUTIVE 3D PILL ACTION BUTTONS -->\s*<div class="top-pill-actions-container".*?</div>', '', content, flags=re.DOTALL)
        
        # 2. Insert dedicated pill bar right above <header id="main-header">
        if "EXECUTIVE 3D PILL ACTION BAR" not in content and '<header id="main-header"' in content:
            ...
            with open(fpath, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"Cleaned nav overlap & added dedicated 3D pill action bar to {filename}!")
        else:
            with open(fpath, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"Cleaned nav overlap in {filename}!")

print("All headers cleaned and updated successfully with zero text overlap!")
