...
# Remove old lower testimonial section from index.html if present
start_marker = "<!-- ============================================================ -->\n  <!-- HIGHLIGHTED GLOBAL CLIENT VIDEO TESTIMONIALS -->"
end_marker = "<!-- PRODUCTS WE SOURCE & EXPORT -->"

if start_marker in content and end_marker in content:
    part1 = content.split(start_marker)[0]
    part2 = content.split(end_marker)[1]
    cleaned_index = part1 + end_marker + part2
else:
    cleaned_index = content

hero_target = "<!-- GLOBAL RFQ DATA TICKER -->"
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(270px, 1fr)); gap: 20px; text-align: left;">
              ...
              <div style="background: #0f172a; border: 1px solid rgba(255,255,255,0.1); border-radius: 14px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.4);">
                  <div style="position: relative; width: 100%; padding-bottom: 56.25%; background: #000;">
                      <video controls playsinline webkit-playsinline preload="none" style="position: absolute; top:0; left:0; width:100%; height:100%; object-fit: cover;">
              ...
              <div style="background: #0f172a; border: 1px solid rgba(255,255,255,0.1); border-radius: 14px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.4);">
                  ...
                  <div style="padding: 16px;">
                      <h4 style="color: #fff; font-size: 15px; font-weight: 700; margin: 0 0 4px; display: flex; align-items: center; justify-content: space-between;">
                          <span>Tariq Hossain</span> <span style="font-size: 14px;">🇧🇩</span>
                      </h4>
                      <p style="color: var(--gold); font-size: 12px; font-weight: 600; margin: 0 0 6px;">Grain &amp; Rice Importer</p>
                      <span style="font-size: 11px; background: rgba(59, 130, 246, 0.2); color: #60a5fa; padding: 2px 8px; border-radius: 4px; font-weight: 700;">Verified Grain Buyer</span>
                  </div>
              </div>
...
# Insert testimonial section in index.html right after hero_target
if hero_target in cleaned_index:
    new_index = cleaned_index.replace(hero_target, hero_target + "\n" + testimonial_html)
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(new_index)
    print("Placed Testimonials section right after Hero section in index.html!")

# Also insert full Testimonials section in membership.html right before OFFICIAL BUSINESS IDENTITY
with open("membership.html", "r", encoding="utf-8") as f:
    mcontent = f.read()

m_target = "<!-- OFFICIAL BUSINESS IDENTITY SECTION -->"
if m_target in mcontent:
    new_mcontent = mcontent.replace(m_target, testimonial_html + "\n" + m_target)
    with open("membership.html", "w", encoding="utf-8") as f:
        f.write(new_mcontent)
    print("Inserted full 14 Testimonials grid in membership.html!")
